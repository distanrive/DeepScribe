...
func _probe_worker(python: String) -> void:
	var output: Array = []
	_probe_exit = OS.execute(python, ["-c", _probe_code()], output, true)
	_probe_output = "\n".join(output)


## 依赖自检要 import 的模块列表。
##
## **必须与本项目后端的真实依赖一致** —— 模板默认是它自己 demo 用的一组
## （websockets/numpy/scipy），DeepScribe 的后端只用 websockets；照抄模板会把
## 「装了 websockets、没装 scipy」的正常环境判成「Python 环境不可用」，
## 于是明明能跑的后端永远拉不起来。
##
## 由 `project.godot` 的 `backend/probe` 配置（逗号分隔），换了后端依赖记得一起改。
func _probe_modules() -> String:
	var raw := str(ProjectSettings.get_setting("backend/probe", "websockets, numpy, scipy"))
	var parts := PackedStringArray()
	for m in raw.split(","):
		var name := m.strip_edges()
		if not name.is_empty():
			parts.append(name)
	return ", ".join(parts)


func _probe_code() -> String:
	return "import " + _probe_modules()
...
func _fail(reason: String) -> void:
	# 依赖自检 / 拉起失败，但这些都不重要 —— 只要此刻已经连着后端，链路就是通的。
	# 典型场景：端口上已经有一个外部起好的后端，而解释器自检因为环境差异报了错；
	# 这时把状态条刷成「后端启动失败」纯属误报（用户会去追一个不存在的故障）。
	if NetClient.is_open():
		_set_running()
		return
