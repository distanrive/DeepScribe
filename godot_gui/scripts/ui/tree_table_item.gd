...
var _colors: Dictionary = {}            # col -> Color（只对显式设过色的列生效）
var _tooltips: Dictionary = {}          # col -> String（单元格悬停提示）
var _expanded := false
...
## **会清掉逐列颜色**：整行换数据时旧颜色不再对应新内容，留着会串色。
...
## 给某一列的文本单独上色（状态列用）。传 alpha = 0 的颜色可恢复该列默认色。
## 只有显式设过色的列会偏离主题的 text_color，其余列不受影响。
func set_cell_color(col: int, color: Color) -> TreeTableItem:
	if color.a <= 0.0:
		_colors.erase(col)
	else:
		_colors[col] = color
...
## 该列的自定义色；没设过时返回**全透明**（调用方据此回退到默认色）。
func get_cell_color(col: int) -> Color:
	return _colors.get(col, Color(0, 0, 0, 0))


## 给某一列加悬停提示（显示不下又需要看全的内容，比如完整文件路径）。
## 传空串清除。`TreeTable` 按鼠标位置查出所在行与列后交给它。
func set_cell_tooltip(col: int, text: String) -> TreeTableItem:
	if text.is_empty():
		_tooltips.erase(col)
	else:
		_tooltips[col] = text
...
func get_cell_tooltip(col: int) -> String:
	return _tooltips.get(col, "")
