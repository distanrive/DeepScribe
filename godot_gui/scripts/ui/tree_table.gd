...
## 悬停提示：按鼠标位置定位到「哪一行、哪一列」，取 `TreeTableItem` 上配的文本。
## Godot 的 Control 只有一个 `tooltip_text`，逐单元格提示只能走这个回调。
func _get_tooltip(at_position: Vector2) -> String:
	var i := _row_at(at_position)
	if i < 0:
		return ""
	var item := _visible[i]
	var col := _col_at(at_position.x)
	return item.get_cell_tooltip(col) if col >= 0 else ""


## 命中第几列（-1 = 没命中）。
func _col_at(x: float) -> int:
	var left := 0.0
	for c in range(_titles.size()):
		left += _col_w(c)
		if x < left:
			return c
	return -1
...
func _draw() -> void:
	...
	for item in _visible:
		...
		draw_string(font, Vector2(name_x, baseline), _cell_text(item, 0),
				...
				maxf(_col_w(0) - name_x - pad, 0.0), fs, _cell_color(item, 0, text))

		# 其余列
		var cx := _col_w(0)
		for c in range(1, _titles.size()):
			draw_string(font, Vector2(cx + pad, baseline), _cell_text(item, c),
					HORIZONTAL_ALIGNMENT_LEFT, _col_w(c) - pad * 2.0, fs,
					_cell_color(item, c, text))
			cx += _col_w(c)
		y += row_h
...
## 该单元格的文字色：行上显式设过就用它（状态列），否则回退到主题的 text_color。
## 「设过」的判定是 alpha > 0 —— 见 `TreeTableItem.set_cell_color`。
func _cell_color(item: TreeTableItem, col: int, fallback: Color) -> Color:
	var c := item.get_cell_color(col)
	return c if c.a > 0.0 else fallback
