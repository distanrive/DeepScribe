...
static func _custom_types(t: Theme) -> void:
	...
	# 文件拖放框（FileDropBox / FileDropZone —— 后者是多文件版，共用同一套配色）
	_add_drop_colors(t, "FileDropBox")
	_add_drop_colors(t, "FileDropZone")
...
static func _type_variations(t: Theme) -> void:
	...
	# 侧边栏导航项 + 侧边栏面板
	_add_nav_variation(t)
	_add_sidebar_panel(t)
	...
	# 带语义色的注释：表单里「留空则不改动」「已保存但解不开」这类提示需要
	# 用颜色区分轻重，但字号仍是 Caption —— Status* 那组是 FONT_MD，套不上。
	_add_label_variation(t, "CaptionWarn", ThemePalette.WARNING, ThemePalette.FONT_XS)
	_add_label_variation(t, "CaptionError", ThemePalette.DANGER, ThemePalette.FONT_XS)
...
## 拖放框（`FileDropBox` 单文件 / `FileDropZone` 多文件）的配色。
## 两者外观一致、语义不同（一个收一个路径，一个收一批 PDF），所以共用同一组颜色。
static func _add_drop_colors(t: Theme, type_name: String) -> void:
	t.set_color("bg_color", type_name, ThemePalette.SURFACE)
	t.set_color("bg_hover_color", type_name, ThemePalette.SURFACE_ALT)
	t.set_color("bg_drop_color", type_name, ThemePalette.ACCENT_SOFT)
	t.set_color("border_color", type_name, ThemePalette.BORDER_STRONG)
	t.set_color("border_hover_color", type_name, ThemePalette.ACCENT)
	t.set_color("border_drop_color", type_name, ThemePalette.ACCENT)
...
## 侧边栏导航项（`NavButton`）：整页切换的导航用，配 `toggle_mode + ButtonGroup`。
##
## 与 `GhostButton` 的区别不是颜色而是语义：GhostButton 是「卡片/工具栏里的次要操作」，
## 悬停才浮出浅底；NavButton 是常驻条目，靠**按下态**表达「当前在哪一页」——
## 所以 `pressed` / `hover_pressed` 两个 stylebox 必须是选中底色，不能留空。
static func _add_nav_variation(t: Theme) -> void:
	t.set_type_variation("NavButton", "Button")
	t.set_color("font_color", "NavButton", ThemePalette.TEXT_SEC)
	t.set_color("font_hover_color", "NavButton", ThemePalette.TEXT)
	t.set_color("font_pressed_color", "NavButton", ThemePalette.ACCENT)
	t.set_color("font_focus_color", "NavButton", ThemePalette.TEXT_SEC)
	t.set_color("font_hover_pressed_color", "NavButton", ThemePalette.ACCENT)
	t.set_color("font_disabled_color", "NavButton", ThemePalette.TEXT_DIS)
	t.set_font_size("font_size", "NavButton", ThemePalette.FONT_LG)

	var ph := ThemePalette.PAD_NAV_H
	var pv := ThemePalette.PAD_NAV_V
	var r := ThemePalette.RADIUS_SM
	var empty := _sb(_TRANSPARENT, _TRANSPARENT, r, 0, ph, pv)
	var hover := _sb(ThemePalette.SURFACE_ALT, _TRANSPARENT, r, 0, ph, pv)
	var checked := _sb(ThemePalette.ACCENT_SOFT, _TRANSPARENT, r, 0, ph, pv)
	var checked_hover := _sb(ThemePalette.ACCENT_SOFT_HOVER, _TRANSPARENT, r, 0, ph, pv)
	_apply_button_styles(t, "NavButton", empty, hover, checked, empty,
			_outline(ThemePalette.ACCENT, r))
	t.set_stylebox("hover_pressed", "NavButton", checked_hover)


## 侧边栏面板（`SidebarPanel`）：白底、**无圆角**、只在右侧画一条分隔线。
##
## 默认的 PanelContainer 是圆角卡片 + 四边描边，撑满整条侧边时会显得像一张浮起来的
## 卡片，圆角和左边框都贴在窗口边缘上。这里用 StyleBoxFlat 的单边 `border_width_right`
## 改成「贴边的分隔条」。
static func _add_sidebar_panel(t: Theme) -> void:
	t.set_type_variation("SidebarPanel", "PanelContainer")
	var sb := StyleBoxFlat.new()
	sb.bg_color = ThemePalette.SURFACE
	sb.border_color = ThemePalette.BORDER
	sb.border_width_right = 1
	sb.content_margin_left = ThemePalette.NAV_PAD
	sb.content_margin_right = ThemePalette.NAV_PAD
	sb.content_margin_top = ThemePalette.NAV_PAD
	sb.content_margin_bottom = ThemePalette.NAV_PAD
	t.set_stylebox("panel", "SidebarPanel", sb)
